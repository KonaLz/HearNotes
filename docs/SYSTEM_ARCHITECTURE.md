...
`tauriInvoke(command, args)` 必须原样传递参数对象；导出命令依赖文件名、格式和内容参数。IPC 可能拒绝为字符串，界面统一用 `errorText` 处理，避免空白错误。结果警告、逐段待核标记及已知校验错误通过 `i18n.js` 中的 `systemText` 映射翻译，旧记录无需重转写。切换语言保留播放位置、筛选条件及未保存修改。
